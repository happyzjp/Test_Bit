...
# Fix pandas compatibility issue with bittensor (must be before importing bittensor)
try:
    import pandas as pd
    ...
    def verify_signature(
        ...
        try:
            ...
            if abs(current_time - request_time) > 300:
                logger.warning(f"Signature timestamp expired: {current_time - request_time}s")
                return False
    ...
    def verify_response(self, response_data: Dict) -> bool:
        try:
            ...
            if abs(current_time - response_time) > 300:
                logger.warning(f"Response signature timestamp expired: {current_time - response_time}s")
                return False
